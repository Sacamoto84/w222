...
/*! \class RapidJSON::Распределитель
    ...
    Обратите внимание, чтоMalloc()иRealloc()нестатические, аFree()— статические.

    Поэтому, если дистрибьютору необходимо поддерживать Free(), необходимо ему поместить свой указатель в
    заголовок блока памяти.

\code
концепция Распределитель {
    static const bool kNeedFree;    //!< Нужно ли этому распределителю выходаFree().

    // Выделить блок памяти.
    // \param размер блока памяти в байтах.
    // \возвращает указатель на блок памяти.
    void* Malloc (размерsize_t);

    // Изменение размера блока памяти.
    // \param originalPtr Указатель текущего блока памяти. Нулевой указатель разрешен.
    // \param originalSize Текущий размер в байтах. (Проблема проектирования: поскольку какой-то распределитель может этого не учитывать, то переход к нему может сэкономить память.)
    ...
    статическая неактивность Free (void *ptr);
};
\endcode
*/
...
/*! Этот распределитель выделяет блоки памяти из заранее выделенных фрагментов памяти.
    ...
    Он не освобождает блоки памяти. АRealloc()предлагает только новую память.

    Фрагменты памяти извлекаются с помощью BaseAllocator, который по умолчанию является CrtAllocator.

    Пользователь также может предоставить буфер в качестве первого фрагмента.

    Если пользовательский буфер заполняется, BaseAllocator предлагает дополнительные фрагменты.

    Пользовательский буфер не освобождается этим распределителем.

    \tparam BaseAllocator тип распределения для выделения фрагментов памяти. По умолчанию — CrtAllocator.
    \note реализовано распределение
*/
...
public:
    static const bool kNeedFree = false;    //!< Сообщите пользователям, что с помощью этого распределителя не нужен адаптерFree(). (концепция распределителя)
    ...
    */
    ...
    /*! Пользовательский буфер будет использоваться в первую очередь. Когда он заполнен, пул памяти выделяет новый фрагмент с размером фрагмента.
        ...
        \param buffer Буфер предоставлен пользователем.
    ...
    */
    ...
    void* Realloc(void* originalPtr, size_t originalSize, size_t newSize) {
        ...
        // Процесс Realloc: копировать и копировать память, не освобождая исходный буфер.
...
private:
    ...
    /*! \param емкость Емкость чанка в байтах.
        \Верните истину в случае успеха.
    */
...
#ifdef __GNUC__
RAPIDJSON_DIAG_PUSH
RAPIDJSON_DIAG_OFF(effc++) // std::allocator можно безопасно соблюдать
#endif
