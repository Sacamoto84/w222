...
// Вызовы рисования nanovgAPIдолжны быть заключены вnvgBeginFrame()и nvgEndFrame().
// nvgBeginFrame() определяет размер окна для рендеринга относительно текущего момента.
// установите область просмотра (т.е. glViewport на бэкэндахGL). Соотношение пикселей устройства позволяет
// управлять рендерингом на устройствах Hi-DPI.
// Например, GLFW возвращает два измерения открытого окна: размер окна и
// размер буфера кадра. В этом случае вам необходимо установить windowWidth/Height в размер окна.
...
//
// Составные операции в NanoVG смоделированы по изображениюHTMLCanvasAPIи
// Функция переключения на основе OpenGL (дополнительная информация см. в соответствующих руководствах).
// Цвета в состоянии смешивания имеют предварительно умноженную альфу.

// Устанавливает структурную операцию. Параметр op должен быть одним из NVGcompositeOperation.
...
// Устанавливает составную операцию с пользовательской пиксельной арифметикой дляRGBи альфа-компонентов отдельно. Параметры должны быть одним из NVGblendFactor.
void nvgGlobalCompositeBlendFuncSeparate(NVGcontext * ctx, int srcRGB, int dstRGB, int srcAlpha, int dstAlpha);

//
// Цветовые утилиты
//
// Цвета в NanoVG сохраняются как целые числа без знака в формате ABGR.
...
// Обработка состояния
//
// NanoVG содержит состояние, которое показывает, как будет звучать физический путь.
...
// Для восстановления состояния необходимо использовать адаптер nvgRestore().
...
// с помощью nvgLinearGradient(), nvgBoxGradient(),nvgRadialGradient()и nvgImagePattern().
//
// Текущий стиль рендеринга можно сохранить и восстановить с помощьюnvgSave()и nvgRestore().

// Устанавливает, рисует плавное сглаживание дляnvgStroke()и nvgFill(). Он включен по умолчанию.
...
//   [ky sy ty]
//   [ 0  0  1]
// Where: sx,sy определяют масштабирование, сдвиг kx,ky и трансляцию tx,ty.
// Предполагается, что последняя строка равна 0,0,1 и не сохраняется.
//
// Помимо nvgResetTransform(), есть функция преобразования первого изображения.
// конкретную матрицу преобразования и предварительно умножает на нее текущее преобразование.
//
// Текущую координату системы (преобразование) можно сохранить и восстановить с помощьюnvgSave()иnvgRestore().
...
// Устанавливает преобразование в матрицу skew-x. Угол перемещения в радианах.
...
// Кроме того, вы можете загрузить свое собственное изображение. Загрузка изображения необходимаstb_image.
// Параметр imageFlags представляет собой изменение флагов, определенных в NVGimageFlags.
...
// Краски
//
// Подставки NanoVG четырех типов красок: линейный градиент, прямоугольный градиент, радиальный градиент и узорное изображение.
...
// Для линейного градиента icol задается начальный цвет, a ocol — конечный цвет.
// Градиент преобразуется текущим преобразованием, когда он применяетсяnvgFillPaint()илиnvgStrokePaint().
...
// граница стороны. Параметр icol определяет внутренний цвет, а ocol — внешний цвет градиента.
// Градиент преобразуется текущим преобразованием, когда он применяетсяnvgFillPaint()илиnvgStrokePaint().
NVGpaint nvgBoxGradient(NVGcontext * ctx, float x, float y, float w, float h,
                        float r, float f, NVGcolor icol, NVGcolor ocol);

// Создает и сохраняет радиальный градиент. Параметры (cx,cy) определяют центр, inr и outr определяют
// внутренний и внешний радиус градиента, icol определяет начальный цвет, a ocol — конечный цвет.
// Градиент преобразуется текущим преобразованием, когда он применяетсяnvgFillPaint()илиnvgStrokePaint().
...
// Прямоугольник-ножницы преобразуется текущим преобразованием.
// Note: в случае, если вращение предыдущего ножничного прямоугольника отличается от
...
// Пути
//
// Рисование новых фигур начинается сnvgBeginPath(), при этом очищаются все настройки в данный момент пути.
...
//
// NanoVG использует исключение чет-нечет для рисования рисунка. Твердые фигуры должны располагаться против часовой стрелки.
// обмотка и отверстия должны располагаться против часовой стрелки. Указать извилистость пути можно
// позвонитеnvgPathWinding(). Это особенно полезно для фигурок, которые рисуютсяCCW.
//
// Наконец, вы можете найти путь, используя настоящий стиль заливки, вызвавnvgFill()и обведя его.
// с текущим стилем штриха, вызвавnvgStroke().
//
...
// и дуга рисуется от угла a0 до a1 и движется в направлении dir (NVG_CCWилиNVG_CW).
...
//      nvgFill (вг);
//
// Note: в настоящее время для текста поддерживается только сплошная заливка цветом.
...
// Устанавливает традиционный текст, текущий стиль текста, параметры см. в разделе NVGalign.
...
// Измеряет указанную текстовую надпись. Параметр Границы должен быть указателем с плавающей запятой[4],
...
// Измеряет указанную многотекстовую букву. Параметр Границы должен быть указателем с плавающей запятой[4],
// если должна быть возвращена ограничивающая рамка текста. Значения границ: [xmin,ymin, xmax,ymax]
// Измеренные значения возвращаются в локальном пространстве координат.
