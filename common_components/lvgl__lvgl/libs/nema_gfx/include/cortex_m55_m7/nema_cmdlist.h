...
typedef struct nema_cmdlist_t_ {
    ...
    // секторный круговой кл, связанный
...
/** \brief Определите, в каком списке команда будет поставлена следующей командой.
 *         Привяжите этот список команд как круговой, состоящий из нескольких секторов. Минимум
 *         количество секторов равно 2. Входной номер сектора меньше 2 автоматически устанавливается по умолчанию.
 *         на 2 сектора. Каждый сектор является суб-CL и должен иметь размер не менее 512 байт.
 *         CL никогда не заполняется, никогда не расширяется, он может быть неявно отправлен,
 *         его нельзя использовать повторно. Никакие другие CL не должны быть представлены.
 *         в то время как секторный круг CL связан
 *
 * \param cl Указатель на список команд
 * \param columns Количество секторов, из которых составлен список команд.
 *
 */
