...
/* Типы и определения для egl.h, сведения с платформы
 *
 * Усыновители могут изменитьkhrplatform.hи этот файл в соответствии со своей платформой.
 * Вам рекомендуется отправить все изменения в группу Хронос, чтобы
 * они могут быть включены в будущие версии этого файла.  Пожалуйста, отправьте изменения
 * оформив вопрос или запрос на включение в общедоступный реестр KhronosEGLпо адресу
 ...
 *
 * EGLAPI возвращаемый типEGLAPIENTRYeglFunction(аргументы);
 * typedef тип возвращаемого значения (EXPAPIENTRYPPFNEGLFUNCTIONPROC ) (аргументы);
 *
 * KHRONOS_APICALL иKHRONOS_APIENTRYпрошли вKHR/khrplatform.h.
 */
 ...
 * представляют собой типы псевдонимов, составленные из оконной системы, таких как X Display * или
 * Контекст устройства Windows. Они должны быть привязаны к конкретной платформе.
 * код ниже. Версии Native*Type с префиксомEGLсовпадают.
 * типы, переименованные в EGL 1.3, поэтому все типы в API начинаются с «EGL».
 *
 * KhronosSTRONGLYRECOMMENDS , что вы определяете определение по умолчанию.
...
/* Определите EGLint. Это должен быть целочисленный тип для знакомого, достаточно большой, чтобы его можно было сохранить.
 ...
 * Дескрипторы имеют 64-битный тип, тогда EGLint следует определять как подписанный 64-битный тип.
 * целочисленный тип.
 */
