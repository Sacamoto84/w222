...
/**
 * Повторно сканировать все подключенные устройства evdev и сохранить совместимые устройства в массиве статических устройств для установки быстрого фильтра.
...
static bool _rescan_devices(void)
{
    ...
    while((ent = readdir(dir))) {
        ...
        /* Указатель устройства гарантированно действителен до следующего libinput_dispatch. потому что мы не отправляем события
 ...
 * @param path путь к файлу устройства
 * @param capabilities возможности ввода устройства
 * @return true, если операция завершилась успешно
 */
static bool _add_scanned_device(char * path, lv_libinput_capability capabilities)
{
...
static void _read_pointer(lv_libinput_t * dsc, struct libinput_event * event)
{
    ...
    /* Нам нужны размеры неповернутого дисплея непосредственно из драйвера, потому что libinput не будет учитываться.
    ...
    switch(type) {
        ...
        case LIBINPUT_EVENT_TOUCH_UP:
            /*
             * Мы не применяем «мультитач», но поддержки libinput. Чтобы быстро напечатать два особых переключателя
