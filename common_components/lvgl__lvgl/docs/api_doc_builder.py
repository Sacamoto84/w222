...
# `doxy_src_file_ext_list` должен
# - сопоставить расширение в список DoxyfileFILE_PATTERNSи
...
# Исключено из doxygen через конфигурацию элемента EXCLUDE.  (Этот конфиг
# элемент установлен в`doxygen_xml.py`.)
_cfg_exclude_list = ['lv_conf_internal.h']

# Многострочное совпадение `` API + новая строка + \*\*\* + пробел `` .
# NB : `` \s* `` в конце заставляет регулярное выражение соответствовать пробелу
# в конце, включая все \r\n.  Это будет соответствовать UP TO :
# - следующий непустой символ (может быть много пустых строк) или
# - до конца файла, в зависимости от того, что наступит раньше.
_re_api_section_sep = re.compile(r'(?mi)^API *\r?\n^\*\*\*\s*')

# Регулярное выражение для идентификации директивы «..APIравно:lv_obj_t,lv_array_t».
_re_api_equals = re.compile(r'(?mi)^\s*\.\.\s+API\s+equals:\s*([\w,\s]+)')

# Регулярное выражение для идентификации директивы «..APIначинается с:lv_obj,lv_array».
_re_api_startswith = re.compile(r'(?mi)^\s*\.\.\s+API\s+startswith:\s*([\w,\s]+)')

# Регулярное выражение для сопоставления разделителей элементов списка запятыми и пробелами в нескольких строках.
_re_multi_line_comma_sep = re.compile(r'(?m)[,\s]+')

# Регулярное выражение для идентификации гиперссылок, добавленных редактором: :ref: `lv_obj_h`
_re_editor_added_hyperlink = re.compile(r'^\s*:ref:`([\w/\.]+)`')

# Разделитель для обозначения места, куда этот скрипт добавил гиперссылки.
_auto_gen_sep = '.. Autogenerated'

# Список словарей символов
_defines = {}
_enums = {}
_variables = {}
_namespaces = {}
...
def old_clean_name(nme):
    ...
    # Обработать ошибку:
    # AttributeError: Объект «NoneType» не имеет атрибута «startswith».
    if nme is None:
        return nme
...
# Определения:
# - "section" => Имя "abc_def" имеет 2 раздела.
# - N = количество разделов в`item_name`.
# После удаления ведущих '_lv_', 'lv_' и конечного '_t' из `obj_name`,
# Характер первых N «разделов»`obj_name`соответствует `item_name`
# (case sensitive)?
def old_is_name_match(item_name, obj_name):
    # Обработать ошибку:
    # AttributeError: объект «NoneType» не имеет атрибута «split».
    if obj_name is None:
        return False
    ...
    # Отклонить (False), если`obj_name`не имеет количества разделов,`item_name`.
...
def _conditionally_add_hyperlink(obj, genned_link_set: set, exclude_set: set):
    ...
    if obj.file_name is not None:
        # Имя ссылки должно выглядеть следующим образом: draw/sw/blend/lv_draw_sw_blend.h.
        # в том числе при работе под Windows.
        # `obj.file_name` содержит слово, которое выглядит следующим образом:
        # '/путь/к/lvgl/src/draw/sw/blend/lv_draw_sw_blend.h'
        # Примечание: мы не можем использовать «lvgl/src», поскольку нет никаких гарантий.
        # что каталог репозитория пользователя LVGL будет называться так.
        src_root = '/src/'
    ...
    for partial_symbol in strings:
        for symbol_dict in _symbol_dict_list:
            for key in symbol_dict:
                if key is None:
                    # В словаре`enums`есть ключ`None`, который содержит
                    # все значения перечислений из всех безымянных перечислений, и каждое
                    # Значение перечисления имеет поле `file_name`.
...
def _process_end_of_eligible_doc(b: str, rst_file: str) -> (str, str, int):
    ...
    # 4. Удаляем`_auto_gen_sep`и все что после него:
    new_b = b.split(_auto_gen_sep, 1)[0]

    # 5. С помощью`new_b`любые страницы гиперссылки, добавленные редактором, можно установить:
    # `editor_link_set` .
    ...
    # 6. Если присутствует`_re_api_equals`:
    # - построить список символов
    # - вычислять список гиперссылок из символов
    # — добавить гиперссылки на `genned_link_set`, если их нет в `editor_link_set`.
    ...
    # 7. Если присутствует`_re_api_startswith`:
    # - построить список символов
    # - вычислять список гиперссылок из символов
    # — добавить гиперссылки на `genned_link_set`, если их нет в `editor_link_set`.
    ...
    # 8. При отсутствии какой-либо из приведенных выше пользовательских директив используйте строчные буквы
    # основу имени файла и префикс " lv_ " и попробуйте
    # «API начинается с него».
    ...
    # 10. Возврат кортежа: (new_B, C,links_added_count).
...
def _process_one_file(rst_file: str):
    ...
    if eligible_match is not None:
        # Соответствует критериям (найден раздел API).
        i = eligible_match.end()
        # Разделить сразу после заголовка раздела API + пробел.
        a = rst_contents[:i]
        b = rst_contents[i:]
...
def _build_one_local_dictionary(local_dict, remote_dict):
    ...
    for symbol in remote_dict:
        # Примечание: символ`None`на самом деле является допустимым символом в
        # Словарь`enums`, постоянно все символы перечисляемых значений.
        # для перечислений без имен.
...
def _add_hyperlinks_to_eligible_files(intermediate_dir: str,
    ...
    """
    if new_algorithm:
        # Заполнить набор локальных словарей символов с помощью
        # символы WITHOUT любые префиксы «_».
        _build_local_symbol_dictionaries()

        # Составить список файлов`.rst`.
        file_list = []
        ...
        # Для каждого файла`.rst`указаны соответствующие гиперссылки API.
        for rst_file in file_list:
            ...
            if links_added_count > 0:
                ...
                # анонс(__file__, f'Подследующий документ: [{rst_file}].')
        ...
        for folder in doc_rel_paths:
            # Получите список файлов «.rst», эксперименты «index.rst».
            rst_files = list(
            ...
            # Для каждого. RST в этом каталоге...
            for stem, path in rst_files:
                # Начните с пустого набора.
                html_includes = set()

                # Постройте набор`html_includes`в виде списка кортежей, содержащих
                # (имя,html_file).  Пример: «draw.rst» имеет`stem`== «рисовать»,
                # и генерирует список кортежей из файлов .H, где совпадают
                # Были найдены имена элементов C-кода.  Пример:
                # {('lv_draw_line', 'нарисовать\\lv_draw_line.html'),
                ...
                if html_includes:
                    # Преобразовать набор`html_includes`в список содержащих строк
                    # Синтаксис гиперссылки Sphinx «имена целей ссылок».  Пример сверху:
                    # [':ref:`lv_draw_line_h`\n',
                    # ':ref:`lv_draw_sdl_h`\n',
                    ...
                    # Преобразуйте этот список в один текст гиперссылки Sphinx.
                    # имена целей с пустыми строками между ними.
                    # :ссылка: `lv_draw_line_h`
                    #
                    # :ссылка: `lv_draw_sdl_h`
                    #
                    # :ссылка: `lv_draw_sw_blend_to_i1_h`
                    #
                    # и т. д.
                    output = ('\n'.join(html_includes)) + '\n'

                    # Добавьте эту строку в источник. Файл RST обрабатывается.
...
def _create_rst_files_for_dir(src_root_dir_len: int,
    ...
    """
    indent = '    '
    # Оставьте только подпуть в раздел «lvgl/src/».
    # Пример: «рисовать/переключить/смешать»
    sub_path = src_dir_bep[src_root_dir_len:]
    # создать полный путь к выходному каталогу для файла «index.rst».
    # Пример: '/path/to/lvgl/src/docs/intermediate/API /draw/sw/blend'
    out_dir = str(os.path.join(out_root_dir, sub_path))

    # Убедитесь, что каталог существует.  Необходимо создать несколько каталогов MAY.
    # поскольку файлы`.rst`возвращаются в последовательности.
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)

    # Только для каталога верхних уровней... (последний созданныйindex.rst,
    # поскольку они создаются в восходящей последовательности)
    if len(sub_path) == 0 and out_dir.endswith(os.sep):
        # Обрезать косую черту из `out_dir`.
        out_dir = out_dir[:-1]
...
def _recursively_create_api_rst_files(depth: int,
    ...
    # Для каждого «вещи», найденного в`src_dir_bep`, создайте следующее:
    # `elig_sub_dirs` и`elig_h_files`.
    # При изменении дизайна мы включаем файлы с «частным»
    # в их именах.  Причина: продвинутые пользователи, которым необходимо использовать
    # для структур, определенных внутри, потребуется документация
    # на этих страницах API!
    for dir_item in os.listdir(src_dir_bep):
        ...
        if os.path.isdir(path_bep):
            ...
            # `ext` преобразуется в нижний регистр, что приводит к случайному изменению регистра.
            # в расширениях для Windows этот алгоритм не мешает.
    ...
    if elig_h_file_count > 0:
        ...
        # Очевидно, что реализация`listdir()`в Linux-Python невозможна.
        # автоматически создавать отсортированный список.
        elig_sub_dirs.sort()
        elig_h_files.sort()

        # представитьindex.rstплюс . RSTдля любого файла .H* непосредственно в каталоге.
...
def build_api_docs(lvgl_src_dir, intermediate_dir, doxyfile_src_file, *doc_rel_paths):
    ...
    """
    # ---------------------------------------------------------------------
    # - Генерировать Doxyfile, заменяющий токены,
    # - запустите Doxygen, генерирующийXML, и
    # - загрузить сгенерированныйXMLиз результатов Doxygen.
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Генерировать. Файлы RST для страниц API.
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Для каждой записи каталога в массиве`doc_rel_paths`...
    # - добавить гиперссылки API в файлы . RST файлы в каталогах переданного массива.
    # ---------------------------------------------------------------------
