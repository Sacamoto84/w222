...
/**
 * Это файловая система, доступная только для чтения, которая использует отсортированную хеш-таблицу для поиска.
 * записи в монолитном двоичном формате. Бинарный файл подключается командой mkfrogfs.
 * инструмент, который поставляется с этим исходным дистрибутивом.
 */
...
// Возвращает текущее или следующее по величине число, кратное 4.
...
const frogfs_entry_t *frogfs_get_entry(const frogfs_fs_t *fs, const char *path)
{
    ...
    /* переместить e в первое совпадение */
    while (middle > 0) {
        e = fs->hash + middle;
        if ((e - 1)->hash != hash) {
            break;
    ...
    /* пройтись по кандидатам и найти совпадение */
