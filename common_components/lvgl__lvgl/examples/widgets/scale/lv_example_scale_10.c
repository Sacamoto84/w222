...
{
    if(hr < 117) return lv_palette_main(LV_PALETTE_GREY); /* Зона 1 */
    else if(hr < 135) return lv_palette_main(LV_PALETTE_BLUE); /* Зона 2 */
    else if(hr < 158) return lv_palette_main(LV_PALETTE_GREEN); /* Зона 3 */
    else if(hr < 176) return lv_palette_main(LV_PALETTE_ORANGE); /* Зона 4 */
    else return lv_palette_main(LV_PALETTE_RED); /* Зона 5 */
}
...
static void hr_anim_timer_cb(lv_timer_t * timer)
{
    ...
    /* Обновить цвет текста в зависимости от зоны */
