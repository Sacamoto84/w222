...
#ifdef ESP_PLATFORM
    #define DIR FF_DIR  /* ESP IDF определяет тип `DIR` как `FF_DIR` в своей версии ff.h. Используйте `FF_DIR` также в LVGL. */
#endif
...
static void fs_init(void)
{
    /*Инициализируйте картуSDи саму FatFS.
     *Лучше сделать это в своем коде, чтобы сохранить эту библиотеку нетронутой и упростить обновление.*/
}
 ...
 * @param drv       указатель на драйвер, которому принадлежит эта функция
 * @param file_p    указатель на переменную FIL. (открывается с помощьюfs_open)
 ...
 * @param drv       указатель на драйвер, которому принадлежит эта функция
 * @param file_p    указатель на переменную FIL.
 ...
 * @param whence    поддерживается только LV_SEEK_SET
 * @return LV_FS_RES_OK: ошибок нет, файл прочитан
 *         любая ошибка из перечисления lv_fs_res_t
 */
static lv_fs_res_t fs_seek(lv_fs_drv_t * drv, void * file_p, uint32_t pos, lv_fs_whence_t whence)
...
/**
 * Инициализируйте переменную DIR для чтения каталога.
 * @param drv   указатель на драйвер, которому принадлежит эта функция
 * @param path  путь к каталогу
 * @return указатель на инициализированную переменную DIR
 */
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
    LV_UNUSED(drv);
 ...
 * @param drv       указатель на драйвер, которому принадлежит эта функция
 * @param dir_p     указатель на инициализированную переменную DIR
...
static lv_fs_res_t fs_dir_read(lv_fs_drv_t * drv, void * dir_p, char * fn, uint32_t fn_len)
{
    ...
    do {
        ...
        if(fno.fname[0] == 0) break; /* Конец каталога */
