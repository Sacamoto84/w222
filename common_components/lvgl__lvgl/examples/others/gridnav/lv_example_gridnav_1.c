...
/**
 * Демонстрация базовой навигации по сетке
 */
void lv_example_gridnav_1(void)
{
    ...
    for(i = 0; i < 10; i++) {
        ...
        lv_group_remove_obj(obj);   /*Не требуется, вместо этого мы используем GridNav.*/
    ...
    /* Создайте второй контейнер с режимом навигации по сетке опрокидывания.*/
    ...
    /*Добавьте и разместите детей вручную*/
    lv_obj_t * ta = lv_textarea_create(cont2);
    lv_obj_set_size(ta, lv_pct(100), 80);
    lv_obj_set_pos(ta, 0, 80);
    lv_group_remove_obj(ta);   /*Не требуется, вместо этого мы используем GridNav.*/

    lv_obj_t * cb = lv_checkbox_create(cont2);
    lv_obj_set_pos(cb, 0, 170);
    lv_group_remove_obj(cb);   /*Не требуется, вместо этого мы используем GridNav.*/

    lv_obj_t * sw1 = lv_switch_create(cont2);
    lv_obj_set_pos(sw1, 0, 200);
    lv_group_remove_obj(sw1);   /*Не требуется, вместо этого мы используем GridNav.*/

    lv_obj_t * sw2 = lv_switch_create(cont2);
    lv_obj_set_pos(sw2, lv_pct(50), 200);
    lv_group_remove_obj(sw2);   /*Не требуется, вместо этого мы используем GridNav.*/
}
