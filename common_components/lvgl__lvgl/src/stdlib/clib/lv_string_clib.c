...
{
    /*strdup использует malloc, поэтому воспользуйтесьlv_malloc, еслиLV_USE_STDLIB_MALLOCнеLV_STDLIB_CLIB. */
    size_t len = lv_strlen(src) + 1;
    char * dst = lv_malloc(len);
    if(dst == NULL) return NULL;

    lv_memcpy(dst, src, len); /*do memcpy работает быстрее, чем strncpy, если сохранить длину*/
    return dst;
}
