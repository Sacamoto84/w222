...
/**
 * @brief Вычисляет ширину объекта LVGL в пикселях на основе его стиля и родителя для заданного свойства `prop`.
 * @param obj Указатель на объект LVGL, ширина которого вычисляется.
 * @param prop Свойство ширины для расчета. Допустимые значения: `LV_STYLE_WIDTH`, `LV_STYLE_MIN_WIDTH` или
 * LV_STYLE_MAX_WIDTH.
...
/**
 * @brief Вычисляет высоту объекта LVGL в пикселях на основе его стиля и родителя для заданного свойства `prop`.
 * @param obj Указатель на объект LVGL, высота которого вычисляется.
 * @param prop На какую высоту стиля рассчитывать. Допустимые значения: `LV_STYLE_HEIGHT`, `LV_STYLE_MIN_HEIGHT` или
 * LV_STYLE_MAX_HEIGHT.
...
#if LV_DRAW_TRANSFORM_USE_MATRIX
    /**
     * При использовании координат вершины глобальной матрицы `clip_area`достигается точность после преобразования.
