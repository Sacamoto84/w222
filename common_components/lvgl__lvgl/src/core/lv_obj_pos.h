 ...
 * @note            например, выравнивание `LV_ALIGN_CENTER`— это смещение от центра родительского элемента.
 ...
 * @param obj       указатель на объект для выравнивания
 * @param base      указатель на другой объект (если используется родительский элемент; `NULL` означает родителя `obj`). 'obj' будет выровнен по нему.
...
/**
 * Получить область, уменьшенную на отступы и ширину границы.
 * @param obj       указатель на объект
 * @note            Положение объекта пересчитывается только при следующей перерисовке. Принудительный перерасчет координат
 *                  позвоните`lv_obj_update_layout(obj)`.
...
/**
 * Ограничить ширину минимальной и максимальной границами. Если минимальная/максимальная ширина указана в процентах, используйте `ref_width`.
 ...
 * @return              ограниченная ширина
 */
int32_t lv_clamp_width(int32_t width, int32_t min_width, int32_t max_width, int32_t ref_width);

/**
 * Ограничить высоту минимальной и максимальной границами. Если минимальная/максимальная высота указана в процентах, используйте `ref_height`.
