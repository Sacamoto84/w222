 ...
 * Это схема сжатия, используемая в формате изображений GIF, и сжатие Unix.
...
struct BitStreamWriter
{
    ...
    int granularity;       //Сумма байтов, выделенных умножается при автоматическом расширении размера вappendBit().
    int currBytePos;       //Текущий байт, в котором отмечена запись, от 0 до bytesAllocated-1.
...
struct Dictionary
{
    ...
    Dictionary()
    {
        /* Первые 256 записей словаря зарезервированы для записи byte/ASCII.
