...
public:
     ...
     *
     * @param [in] degree Значение угла в градусах.
     *
     ...
     *
     * @param factor [in] Значение коэффициента масштабирования. Значение по умолчанию — 1.
     *
     * @retval Result::InsufficientCondition в случае применения пользовательского преобразования.
     * @see Краска:: transform()
     */
     ...
     *
     * @param [дюймы] m Расширенная матрица 3х3.
     */
     ...
     * @param [in] method Метод, используемый для объединения исходного объекта с целевыми.
     */
     ...
     *
     * @param [in] method Устанавливаемый метод колебания.
     *
     * @note Экспериментальный API
     */
...
public:
     ...
     * @param [дюйм] radius Радиус ограничивающего круга.
     *
...
public:
     ...
     * @param [дюймы] radius Радиус дуги.
     * @param [in] startAngle Начальный угол дуги, заданный в градусах и измеренный против часовой стрелки от горизонтальной линии.
     * @param [in] sweep Центрального угла дуги, заданного в градусах и направленного против часовой стрелки от@pstartAngle.
     * @param [in] pie диаграмма Определяет, следует ли рисовать радиусы от центра дуги до ее конечных точек — рисуется, если@cистинно.
     *
     ...
     *
     * @param [дюймы] width Ширина обводки. Значение по умолчанию — 0.
     *
     */
     ...
     * @param [in] simultaneous определяет, как обрезать несколько контуров внутри одной фигуры. Если установлено значение@ctrue (по умолчанию), обрезка применяется одновременно ко всем путям;
...
public:
     ...
     * @param [in] copy предварительным умножением. Если@ctrue, данные изображения умножаются в альфа-канале.
     * @param [in] copy Если@ctrue, данные копируются в локальный буферный механизм, в противном случае — нет.
     *
     * @since 0.9
     */
...
public:
     ...
     *
     * @param [in] text байтовый текст, закодированный строкой utf8 для отображения.
     *
     * @note Экспериментальный API
     */
     ...
     *
     * @param [in] path Путь к файлу шрифта.
     *
     * @retval Result::InvalidArguments В случае, если путь@pнедействителен.
     * @retval Результат::NonSupport При попытке загрузить файл с неизвестным расширением.
     *
     ...
     *
     * @param [in] path к файлу установленного шрифта.
     *
...
public:
     ...
     * @param [in] buffer Шаг растрового изображения – больше или равен@pw.
...
public:
     ...
     * @brief Устанавливает цель рисования для растеризации.
     *
     * @param [in] instance WGPUInstance, контекст для всех остальных объектов wgpu.
     * @param [in] surface WGPUSurface, дескриптор презентабельной поверхности.
     * @param [дюймы] w Ширина поверхности.
     * @param [дюймы] h Высота поверхности.
     * @param [in] device WGPDevice, желаемый дескриптор устройства wgpu. Если это@cnullptr, ThorVG внутренне назначит наличие устройства.
     *
     * @retval Result::InsufficientCondition, если холст выполняет рендеринг. Убедитесь, что холст синхронизирован.
...
public:
     ...
     *
     * @param [in] no Индекс кадровой анимации, который будет находиться в базе данных Bluetooth. Индекс должен быть меньшеtotalFrame().
     *
     ...
     *
     * @param [in] begin сегмента.
     * @param [in] end сегмента конец.
     *
     ...
     *
     * @param [out] begin сегмента.
     * @param [out] end сегмента конец.
     *
     * @retval Result::InsufficientCondition Если анимация не загружена.
     * @retval Result::NonSupport Когда его нельзя анимировать.
     *
...
public:
     ...
     *
     * @param [in] paint Краска, которая будет использоваться в качестве фонового изображения для сохранения красок.
     *
     * @note Экспериментальный API
     */
     ...
     *
     * @param [in] paint Краска, которую необходимо сохранить, со всеми переменами с ней следствием.
     * @param [in] path Путь к файлу, в котором должны быть сохранены данные рисования.
     ...
     *
     * @param [in] animation Сохраняемая анимация, включая все связанные свойства.
     * @param [in] path Путь к файлу, в котором будет сохранена анимация.
     * @param [in] quality Уровень качества кодирования.  @c0 — минимальное значение,@c100 — максимальное значение (рекомендуется).
...
public:
     ...
     *
     * @param [in] picture Узел изображения для обхода внутреннего дерева сцены.
