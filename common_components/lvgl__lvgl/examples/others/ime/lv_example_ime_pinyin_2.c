...
void lv_example_ime_pinyin_2(void)
{
    ...
    //lv_ime_pinyin_set_dict (pinyin_ime,your_dict); // использовать собственный словарь. Если он не установлен, будет использоваться встроенный словарь.
    ...
    /*Создайте клавиатуру и добавьте ее в ime_pinyin.*/
    lv_obj_t * kb = lv_keyboard_create(lv_screen_active());
    lv_keyboard_set_textarea(kb, ta1);

    lv_ime_pinyin_set_keyboard(pinyin_ime, kb);
    lv_ime_pinyin_set_mode(pinyin_ime,
                           LV_IME_PINYIN_MODE_K9);  // Установите режим 9-кнопочного ввода. По умолчанию: режим 26-кнопочного ввода (k26).
    lv_obj_add_event_cb(ta1, ta_event_cb, LV_EVENT_ALL, kb);

    /*Приобретитеcand_panelи отрегулируйте его размер и положение.*/
    lv_obj_t * cand_panel = lv_ime_pinyin_get_cand_panel(pinyin_ime);
    lv_obj_set_size(cand_panel, LV_PCT(100), LV_PCT(10));
    lv_obj_align_to(cand_panel, kb, LV_ALIGN_OUT_TOP_MID, 0, 0);

    /*Попробуйте использоватьime_pinyinдля вывода текста на китайском языке ниже в ta1 выше.*/
