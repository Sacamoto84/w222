...
lv_font_t * lv_binfont_create(const char * path)
{
    ...
    if(!lvgl_load_font(&file, font)) {
        ...
        /*
        * При этом`lvgl_load_font`может вызвать утечку некоторых указателей.
        * Все ненулевые указатели можно считать выделенными и
        * `lv_binfont_destroy` должен разложить их правильно.
        */
...
/*
 * Загружает`lv_font_t`из двойного файла, версия`lv_fs_file_t`.
 *
 ...
 *
 * Когда что-то выходит из строя, он возвращает`false`и сохраняет память на `lv_font_t`.
 * все еще необходимо снять с помощью `lv_binfont_destroy`.
 *
