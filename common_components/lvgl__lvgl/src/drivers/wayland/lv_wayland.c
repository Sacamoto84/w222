...
/* Обратный вызов таймера для обработки событий композитора Wayland без блокировкиUI.
...
/**
 * Получить дескриптор файла воспроизведения Wayland
 * @return Дескриптор файла отображения Wayland
 */
