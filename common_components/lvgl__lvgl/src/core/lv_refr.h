...
/**
 * Немедленно перерисовать недействительные области.
 * Обычно перерисовка периодически выполняется в `lv_timer_handler`, но постоянная блокировка процесса
 * может предотвратить вызов `lv_timer_handler`. В этом случае, если GUI обновляется в процессе
 * (например, индикатор выполнения) эту функцию можно вызвать, когда экран должен быть обновлен.
 * @param disp указатель на дисплей для обновления.  NULL, чтобы обновить все дисплеи.
 */
