 ...
 *      DEFINES
 *********************/
/** Предопределенные клавиши для управления фокусом через `lv_group_send(group, c)` */
typedef enum {
    LV_KEY_UP        = 17,  /*0x11*/
    LV_KEY_DOWN      = 18,  /*0x12*/
...
/**
 * Перевести фокус на виджет (с текущего виджета фокус будет снят).
 * @param obj       указатель на виджет, который должен получить фокус
 */
void lv_group_focus_obj(lv_obj_t * obj);

/**
 * Перевести фокус на следующий виджет в группе (с текущего виджета фокус будет снят).
 * @param group     указатель на группу
 */
void lv_group_focus_next(lv_group_t * group);

/**
 * Перевести фокус на предыдущий виджет в группе (с текущего виджета фокус будет снят).
 * @param group     указатель на группу
 */
...
/**
 * Отправить управляющий символ виджету, который сейчас находится в фокусе группы.
 * @param group     указатель на группу
 * @param c         символ (используйте `LV_KEY_...` для навигации)
 * @return          результат обработки сфокусированным виджетом группы
 */
lv_result_t lv_group_send_data(lv_group_t * group, uint32_t c);

/**
 * Установить для группы callback, который будет вызываться при получении фокуса новым виджетом.
 * @param group         указатель на группу
...
/**
 * Установить для группы callback, который будет вызываться при попытке перейти за край группы.
 * @param group         указатель на группу
...
/**
 * Задать, какой виджет в группе получит фокус после удаления текущего сфокусированного виджета:
 * следующий или предыдущий.
 * @param group         указатель на группу
 * @param policy        новая политика выбора фокуса
 */
 ...
 * @param group         указатель на группу
 * @param edit          true: режим редактирования; false: режим навигации
 */
void lv_group_set_editing(lv_group_t * group, bool edit);

/**
 * Разрешить или запретить циклический переход фокуса между первым и последним виджетом группы.
 * @param group         указатель на группу
...
/**
 * Получить сфокусированный виджет или `NULL`, если фокуса нет.
 * @param group         указатель на группу
...
/**
 * Получить callback, вызываемый при изменении фокуса группы.
 * @param group указатель на группу
...
/**
 * Получить callback, вызываемый при достижении края группы.
 * @param group указатель на группу
 * @return функция обратного вызова или NULL, если не установлено
 */
...
/**
 * Получить текущий режим (редактирование или навигация).
 * @param group         указатель на группу
 * @return              true: режим редактирования; false: режим навигации
 */
bool lv_group_get_editing(const lv_group_t * group);

/**
 * Узнать, разрешен ли циклический переход фокуса между первым и последним виджетом группы.
 * @param group         указатель на группу
 */
...
#if LV_USE_EXT_DATA
/**
 * @brief Прикрепляет к группе внешние пользовательские данные и callback деструктора.
 *
 * Связывает пользовательские данные с группой LVGL и задает функцию деструктора,
 * которая будет автоматически вызвана при удалении группы для корректной очистки
 * связанных ресурсов.
 *
 * @param group      Указатель на группу
 * @param data       Пользовательский указатель на данные, связанные с группой
 * @param free_cb    Callback для очистки `ext_data` при удалении группы.
 *                   Получает `ext_data` в качестве параметра.  `NULL` означает, что очистка не требуется.
 */
