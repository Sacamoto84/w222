...
static void create_ui(void)
{
    ...
    uint32_t i;
    for(i = 0; i < 10; i++) {
        /*Вызов функции, реализованной в привязке, которая возвращает виджет.*/
        ...
        if(i == 0) {
            ...
            lv_anim_set_exec_cb(&a, opa_anim_cb);   /*Передать обратный вызов*/
        ...
        if(i == 3) {
            /*Запустить бесконечную анимацию и удалить эту кнопку позже*/
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, btn);
            lv_anim_set_values(&a, LV_OPA_COVER, LV_OPA_50);
