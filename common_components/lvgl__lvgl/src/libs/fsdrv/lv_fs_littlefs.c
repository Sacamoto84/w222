 ...
 * Открыть каталог
 * @param drv       указатель на драйвер, которому принадлежит эта функция
 * @param path      путь к каталогу, начинающийся с буквы драйвера (например, S:/folder)
 * @return          дескриптор каталога илиNULLв случае ошибки
 */
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
 ...
 * @param drv      указатель на драйвер, которому принадлежит эта функция
 * @param dir_p    указатель на переменную dir_p. (открывается с помощьюfs_dir_open)
 * @return         LV_FS_RES_OK: нет ошибок или ошибок из перечисления @lv_fs_res_t.
 */
static lv_fs_res_t fs_dir_close(lv_fs_drv_t * drv, void * dir_p)
{
 ...
 * @param drv      указатель на драйвер, которому принадлежит эта функция
 * @param dir_p    указатель на переменную file_t.
