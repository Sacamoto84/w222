...
void lv_example_style_11(void)
{
     ...
     * Сделайте это быстро (100 мс) и начните с некоторой задержкой (200 мс).*/
    static lv_style_transition_dsc_t trans_def;
    lv_style_transition_dsc_init(&trans_def, props, lv_anim_path_linear, 100, 200, NULL);

    /* Специальный переход при переходе в нажатое состояние.
     * Сделайте это медленно (500 мс), но начните без задержки.*/
